...
void setQueen(int queens[], int row, int col) {
    for (int i = 0; i < row; i++) {
        int col_diff = abs(queens[i] - col);
        if (col_diff == 0 || col_diff == (row - i)) {
            return;
        }
    }
...
// void solve() {
	...
	// 	ANNOTATE_ITERATION_TASK(setQueen_task);
	// 	setQueen(queens, 0, i);
	// }
	// ANNOTATE_SITE_END();
...
// Применяем оптимизацию
void solve() {
	#pragma omp parallel reduction(+:nrOfSolutions)
    {
...
int main(int argc, char* argv[]) {
	...
	SetProcessAffinityMask(GetCurrentProcess(), 0x3F);
